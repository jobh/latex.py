...
def version():
    import subprocess
    abbrev = 7
    cmd = 'git describe --dirty=+ --all --long --abbrev=%d' % abbrev
    cmd = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
    ver = str(cmd.communicate()[0].strip())
    if ver[-1] == '+':
        abbrev += 1
    ver = ver[-abbrev-2:-1]
    try:
        ...
        pass
    try:
        cmd = 'git rev-parse --short %s' % BASE_REV
        cmd = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
        base_ver = str(cmd.communicate()[0].strip())
        return r'\today{} $\cdot$ \DIFdel{%s} \DIFadd{%s}' % (base_ver, ver)
    except:
        return r'\today{} $\cdot$ '+ver
